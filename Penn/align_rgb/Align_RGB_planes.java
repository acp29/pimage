...
// Align RGB planes  by G.Landini (modified by Andy Penn)
...
// v1.8 12/Jan/2008 Does All Slices
// v1.9 11Jun/2008 Does All Slices fix
// v2.0 11Oct/2019 Switching color checkboxes no longer does 'undo' of alignment (Andy Penn)
...
implements ActionListener, AdjustmentListener, ItemListener, WindowListener, Runnable {
	...
	private void doDialog() {
		...
		bnDoallS = new Button("All slices");
		...
		if (imp.getStackSize()>1) {
			...
			addComponent(pnMain, 17, 1, 1, 1, 5, bnDoallS);

		}

		addComponent(pnMain, 17, 2, 1, 1, 4, bnHelp);
		...
		bnDoallS.addActionListener(this);
		addWindowListener(this);
		// Build panel
		add(pnMain);
		pack();
		setResizable(IJ.isMacintosh()); //Mac bug?
		GUI.center(this);
		setVisible(true);
	...
	public synchronized  void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == bnDoallS){
			...
			IJ.showMessage("Help","Align RGB planes v2.0 by G.Landini (modified by Andrew Penn)\n"+
							"Changes the alignment of the RGB planes independently.\n \n"+
							"\'Red\' \'Green\' and \'Blue\' checkboxes switch ON and OFF the planes\n \n"+
							"Note that when switching planes or checkboxes, the portion of the\n"+
							"previously edited plane left outside the image frame is lost.\n \n"+
							"Rotation, Width and Height changes are interpolated (so there is some\n"+
							"loss of sharpness) and do not retain the image portions outside the\n"+
							"image frame.  You can use the \'Resize2Rotate\' macro to avoid losing\n"+
							"any image data.\n \n"+
							"The \'Rotate\', \'Width\' and \'Height\' sliders set integer values, but\n"+
							"fractional values can also be typed in the entry boxes.\n"+
							"Just make sure you press [RETURN] after the number is typed.\n \n"+
							"The [All Slices] button aligns all the slices in the stack. Please apply\n"+
							"to one plane at a time when using this option.\n \n"+
							"The [Revert], [Rotate], [Width] and [Height] buttons are shown only\n"+
							"with single images, not stacks.\n \n"+
							"The [< Prev] [Next >] and [All Slices] buttons are shown when using\n"+
							"stacks. Do not use the slide bar in the stack window, to navigate but\n"+
							"use the [< Prev] [Next >] buttons instead.");
		}
		notify();
